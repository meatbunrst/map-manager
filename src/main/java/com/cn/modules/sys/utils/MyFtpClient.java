...
public class MyFtpClient {
    ...
    private FTPClient initFtpClient(String remoteDir) throws IOException {
        ...
        boolean loginRet = ftp.login(ftpProperties.getHost(), ftpProperties.getPassword());
        if (!loginRet) {
    ...
    private boolean MKDAndCWD(FTPClient ftp, String remoteDir) throws IOException {
        // 切分出所有子文件夹,按顺序
        if (StringUtils.isNotBlank(remoteDir)&&StringUtils.isNotBlank(ftpProperties.getFtpBasepath())){
            if (remoteDir.startsWith(PART)){
                remoteDir = ftpProperties.getFtpBasepath() + remoteDir;
            }else {
                remoteDir = ftpProperties.getFtpBasepath() + PART+remoteDir;
            }
        }else {
            remoteDir = ftpProperties.getFtpBasepath();
        }
